...
export const driverService = {
  ...
  updateProfile: (payload) => api.patch('/drivers/me', payload).then((response) => response.data),
  getRideRequests: () => api.get('/drivers/ride-requests').then((response) => response.data),
  getRideHistory: () => api.get('/drivers/rides/history').then((response) => response.data),
  getRideDetail: (rideId) => api.get(`/drivers/rides/${rideId}`).then((response) => response.data),
