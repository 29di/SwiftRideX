...
export default function RideRequestCard({ onSubmit, loading = false, initialValues, disabled = false }) {
  ...
  useEffect(() => {
    if (!initialValues) {
      ...
      setDropSuggestions([]);
      setActiveSuggestionIndex({ pickup: -1, drop: -1 });
    ...
    setForm({
      ...
      pickupLat: initialValues.pickupLatitude ?? null,
      pickupLng: initialValues.pickupLongitude ?? null,
      dropAddress: initialValues.dropAddress || '',
      dropLat: initialValues.dropLatitude ?? null,
      dropLng: initialValues.dropLongitude ?? null,
      fare: initialValues.fare ?? '',
    });
  }, [initialValues]);
  ...
  const submitRide = async (event) => {
    ...
    if (disabled) {
      setLocationError('You already have an ongoing ride. Complete it before requesting a new one.');
      return;
    }
    ...
    const trimmedFare = String(form.fare ?? '').trim();
    const normalizedFare = trimmedFare === '' ? undefined : Number(trimmedFare);

    if (normalizedFare !== undefined && (Number.isNaN(normalizedFare) || normalizedFare < 0)) {
      setLocationError('Fare must be a non-negative number. Leave it empty to use backend defaults.');
      return;
    }
    ...
    <Card className="space-y-5">
      ...
      {disabled ? (
        <div className="rounded-2xl border border-amber-400/20 bg-amber-500/10 px-4 py-3 text-sm text-amber-200">
          You already have an ongoing ride. Complete it before requesting a new one.
        </div>
      ) : null}
